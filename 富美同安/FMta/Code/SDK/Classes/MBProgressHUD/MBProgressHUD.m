...
- (void)layoutSubviews {
	...
	CGSize label1Size = MB_TEXTSIZE(label.text, label.font);
	label1Size.width = MIN(label1Size.width, maxWidth);
	totalSize.width = MAX(totalSize.width, label1Size.width);
	totalSize.height += label1Size.height;
	if (label1Size.height > 0.f && indicatorF.size.height > 0.f) {
	...
	if (detailsLabelSize.height > 0.f && (indicatorF.size.height > 0.f || label1Size.height > 0.f)) {
		totalSize.height += kPadding;
	}
	...
	labelF.origin.x = roundf((bounds.size.width - label1Size.width) / 2) + xPos;
	labelF.size = label1Size;
	label.frame = labelF;
