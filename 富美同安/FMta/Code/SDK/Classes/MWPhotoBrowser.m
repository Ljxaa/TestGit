...
- (id)init {
    if ((self = [super init])) {
        [self _initialisation];
            ...
            [backBtn setFrame:CGRectMake(0, 0, 37/2, 34/2)];
            [backBtn setBackgroundImage:[UIImage imageNamed:@"return_white.png"] forState:UIControlStateNormal];
            [backBtn setBackgroundImage:[UIImage imageNamed:@"return_white_press.png"] forState:UIControlStateHighlighted];
...
#pragma mark start by zwb 2014.11.13
#if 0
        ...
        if ([myScDictionary objectForKey:@"IsClickPraise"] == nil || [[myScDictionary objectForKey:@"IsClickPraise"] isEqual:[NSNull null]])
        {
            ClickPraiseBool = NO;
            dImage = [UIImage imageNamed:@"image_dianzan.png"];
        }
        else if ([[myScDictionary objectForKey:@"IsClickPraise"] intValue] ==0)
        {
            ClickPraiseBool = NO;
            dImage = [UIImage imageNamed:@"image_dianzan.png"];
        }
        ...
        {
            ClickPraiseBool = YES;
            dImage = [UIImage imageNamed:@"image_dianzan_press.png"];
        }
        dImage = [dImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        _dianzButton = [[UIBarButtonItem alloc] initWithImage:dImage style:UIBarButtonItemStylePlain target:self action:@selector(collection:)];

        

        UIImage* fImage= [UIImage imageNamed:@"image_weixin.png"];
        fImage  = [fImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        _fenxButton =[[UIBarButtonItem alloc] initWithImage:fImage style:UIBarButtonItemStylePlain target:self action:@selector(fenxAction)];
...
//        {
//            if ([[[scArray objectForKey:[NSString stringWithFormat:@"%d",i]] objectForKey:@"Title"] isEqualToString:[myScDictionary objectForKey:@"Title"]]){
//                scYes = YES;
//            }
//        }
//        if (scYes) {
//            sImage= [UIImage imageNamed:@"image_collect2.png"];
//        }
//        else
//            sImage= [UIImage imageNamed:@"image_collect1.png"];
//        sImage  = [sImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
//        _actionButton =[[UIBarButtonItem alloc] initWithImage:sImage style:UIBarButtonItemStylePlain target:self action:@selector(saveMyPhoto)];
        
//        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//            
//            [SendService IPhoneArticleClick:myScDictionary  setTpye:1];
...
-(void)fenxAction
{
    UIActionSheet* sheet = [[UIActionSheet alloc] initWithTitle:@"微信分享" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"分享朋友圈", @"分享好友", nil];
    sheet.tag = 100;
    [sheet showInView:self.view];
}

-(void)setDion:(NSMutableDictionary*)dictionary
{
    myScDictionary = [[NSMutableDictionary alloc] init];
    [myScDictionary setDictionary:dictionary];
}
#pragma mark -
...
-(void)collection:(UIButton *)sender
{
    NSString *cID = nil;
    NSString *cD = nil;
    if ([[myScDictionary objectForKey:@"ClickPraise"] isEqual:@"2"])
    {
        cID = [NSString stringWithFormat:@"%@|%@|%d",[myScDictionary objectForKey:@"ClassId"],[myScDictionary objectForKey:@"RowID"],1];
        cD = [NSString stringWithFormat:@"%@",[myScDictionary objectForKey:@"PictureMain_id"]];
    }
    else
    {
        cID = [NSString stringWithFormat:@"%@",[myScDictionary objectForKey:@"ClassId"]];
        cD = [NSString stringWithFormat:@"%@",[myScDictionary objectForKey:@"RowID"]];
    }
    
//    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
//        bool dianZ =  [SendService ClickPraise:[myScDictionary objectForKey:@"ClickPraise"] classID:cID CID:cD isClickPraise:!ClickPraiseBool];
//        dispatch_async(dispatch_get_main_queue(), ^{
//            if ((dianZ&&!ClickPraiseBool))
//            {
//                [self WaitView:@"已点赞!"];
//                UIImage* itemImage= [UIImage imageNamed:@"image_dianzan_press.png"];
//                itemImage  = [itemImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
//                [_dianzButton setImage:itemImage];
//                ClickPraiseBool = !ClickPraiseBool;
//                if ([delegate1 respondsToSelector:@selector(goClickPraise1:)])
//                {
//                    [delegate1 goClickPraise1:ClickPraiseBool];
//                }
//            }
//            else if ((!dianZ&&!ClickPraiseBool))
//            {
//                [self WaitView:@"点赞失败!"];
//            }
//            else if ((dianZ&&ClickPraiseBool))
//            {
//                [self WaitView:@"取消点赞!"];
//                UIImage* itemImage= [UIImage imageNamed:@"image_dianzan.png"];
//                itemImage  = [itemImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
//                [_dianzButton setImage:itemImage];
//                ClickPraiseBool = !ClickPraiseBool;
//                if ([delegate1 respondsToSelector:@selector(goClickPraise1:)])
...
//            
//            UIImage* itemImage= [UIImage imageNamed:@"image_collect1.png"];
//            itemImage  = [itemImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
//            [_actionButton setImage:itemImage];
//            [self WaitView:@"取消收藏!"];
//            return;
//        }
//    }
...
//    
//
//    [myScDictionary setObject:locationString forKey:@"myNowTime"];
//    [myScDictionary setObject:[NSString stringWithFormat:@"0"] forKey:@"IsImage"];
//    
//    UIImage* itemImage= [UIImage imageNamed:@"image_collect2.png"];
//    itemImage  = [itemImage imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
//    [_actionButton setImage:itemImage];
//    [scArray setObject:myScDictionary forKey:[NSString stringWithFormat:@"%ld",(unsigned long)scArray.count]];
//    [scArray archiveWithKey:@"DetialsSC"];
//}
...
- (void)performLayout {
    ...
    if (_actionButton && !(!hasItems && !self.navigationItem.rightBarButtonItem)) {
        [items addObject:_fenxButton];
        [items addObject:_dianzButton];
        ...
        if (_dianzButton)
            self.navigationItem.rightBarButtonItem = _dianzButton;
        if (_fenxButton)
            self.navigationItem.rightBarButtonItem = _fenxButton;
        [items addObject:fixedSpace];
    }
...
- (id<MWPhoto>)photoAtIndex:(NSUInteger)index {
    ...
    UITextView *myView =(UITextView*)[self.view viewWithTag:1000];
    if (myView == nil) {
        UITextView *feedBackMsg = [[UITextView alloc] initWithFrame:CGRectMake(10, self.view.bounds.size.height-100-44,self.view.bounds.size.width-20,100)] ;
        feedBackMsg.backgroundColor = [UIColor clearColor];
        feedBackMsg.textColor = [UIColor whiteColor];
        feedBackMsg.editable = YES;
        feedBackMsg.scrollEnabled = YES;
        feedBackMsg.delegate = self;
        feedBackMsg.textAlignment = 1;
        feedBackMsg.tag = 1000;
//        [feedBackMsg setFont:[UIFont fontWithName:@"Arial" size:labelSize]];
        if ([photo respondsToSelector:@selector(caption)]) {
            feedBackMsg.text = [photo caption] ? [photo caption] : @" ";
        }
    ...
    {
        if ([photo respondsToSelector:@selector(caption)]) {
            myView.text = [photo caption] ? [photo caption] : @" ";
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_pagingScrollView != scrollView)
        return;
    // Checks
...
//    _dianzButton.enabled = [[self photoAtIndex:_currentPageIndex] underlyingImage] != nil;
//    _fenxButton.enabled = [[self photoAtIndex:_currentPageIndex] underlyingImage] != nil;
...
- (void)showGrid:(BOOL)animated {
    ...
    else if (self.navigationItem.rightBarButtonItem == _dianzButton) {
        _gridPreviousRightNavItem = _dianzButton;
        [self.navigationItem setRightBarButtonItem:nil animated:YES];
    }
    else if (self.navigationItem.rightBarButtonItem == _fenxButton) {
        _gridPreviousRightNavItem = _fenxButton;
        [self.navigationItem setRightBarButtonItem:nil animated:YES];
    }
    else {
...
- (void)hideGrid {
    ...
    if (_gridPreviousRightNavItem == _dianzButton && _dianzButton) {
        [self.navigationItem setRightBarButtonItem:_gridPreviousRightNavItem animated:YES];
    }
    if (_gridPreviousRightNavItem == _fenxButton && _fenxButton) {
        [self.navigationItem setRightBarButtonItem:_gridPreviousRightNavItem animated:YES];
    }
...
//        if (buttonIndex == 1)
//        {
//            [(AppDelegate*)[[UIApplication sharedApplication] delegate] onSelectSessionScene];
//            
//            [(AppDelegate*)[[UIApplication sharedApplication] delegate] sendLinkContent:myScDictionary];
//        }
//        else if (buttonIndex == 0)
//        {
//            [(AppDelegate*)[[UIApplication sharedApplication] delegate] onSelectTimelineScene];
//            
//            [(AppDelegate*)[[UIApplication sharedApplication] delegate] sendLinkContent:myScDictionary];
//        }
        return;
    }
    else if (actionSheet.tag == ACTION_SHEET_OLD_ACTIONS) {
        // Old Actions
        _actionsSheet = nil;
