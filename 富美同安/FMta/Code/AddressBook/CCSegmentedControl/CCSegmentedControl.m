...
- (void)insertSegmentWithTitle:(NSString *)title atIndex:(NSUInteger)segmentIndex animated:(BOOL)animated
{
    ...
    segmentView.font = [UIFont boldSystemFontOfSize:labelSize-2];
...
- (void)layoutSubviews
{
    ...
    if (_aBool) {
        spaceLeft = CGRectGetWidth(self.bounds) - totalItemWidth*1.2;
    }
    if ((iPadOrIphone) == 2) {
       spaceLeft = CGRectGetWidth(self.bounds) - totalItemWidth*3;
    }
    ...
    else
    {
        ...
        [UIView animateWithDuration:animated ? 0.2 : 0 animations:^
         {
             CGRect myFrame = stainFrame;
             myFrame.origin.y = 0/2;
             myFrame.size.height = 30*(iPadOrIphone);
             self.selectedStainView.frame = myFrame;
         }
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    {
        frame.size.height = 38*(iPadOrIphone);
    }
